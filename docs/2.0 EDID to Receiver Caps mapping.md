...
If some processing happens between a Receiver and its Sink, (e.g. the Receiver is able to consume 4K Flow and downscale it for the Sink with only Full HD in its EDID), Receiver Capabilities MUST reflect not taking EDID as is. This can also be said for Receivers associated with multiple Sinks at the same time.
