...
A device which encapsulates a digital video or/and audio signal (usually, from a physical connection) coverted into IP packets. Alternatively, the device may perform the operation reversed converting IP packets to a digital signal.

### Sink - :question: Destination to be consistent with the API
...
If the Sink acts as a Gateway Device, it's supposed to expose its own EDID instead of EDID of a physical device to take into account any media transformations which the Sink performs. :question:
