...
A mechanism for configuring an IPMX-compatible device and transferring its state. It has an API called "IPMX Device Control" which provides information and control of the device. This conceptual component is the link to the physical EDID medium.
...
Figure 1 shows one context diagram for an IPMX HDMI Gateway System. An IT Staff Person responsible for setting up and administering an IPMX PRO AV System uses an external IPMX Control System. It interacts with the IPMX System via RESTful APIs, SNMPV3, ssh, etc as described in [JT-NM TR-1001-1](https://www.jt-nm.org/tr-1001-1). Note that the architecture presented here does not require a centralized controller (i.e. all NMOS Network Topologies shown above are supported).

| ![Context Diagram for EDID Connection Management with IPMX Control System](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/ipmx_context_diagram.iuml) |
|:--:|
...
| ![Container Diagram for IPMX PRO AV System](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/ipmx_container_diagram.iuml) |
|:--:|
...
Figure 3 shows a component diagram for an IPMX Gateway Encoder. This diagram illustrates the role of a new architectural component shown in tan. This new component is an IPMX Device Controller that provides a proposed IPMX Device Control API to support the User Stories described above. In addition to providing IPMX Device Control API, the IPMX Device Controller interacts with an HDMI Source by presenting an EDID. It also is responsible for the setup of IS-04 resources as shown in the Sequence Diagrams found below. The IPMX Device Control API is analogous to the IS-04 Device Control used for IS-05.

| ![Component Diagram of an IPMX Gateway Encoder](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/ipmx_encoder_component_diagram.iuml) |
|:--:|
| *Figure 3. Component Diagram of an IPMX Gateway Encoder* |

### IPMX Gateway Decoder Component Diagram

Figure 4 shows a component diagram for an IPMX Gateway Decoder. As in the IPMX Gateway Encoder, a new IPMX Device Controller component is present. For an IPMX Gateway Decoder, the IPMX Device Controller is responsible for reading the HDMI Monitor’s EDID, setting up IS-04 Decoder resources including populating the IS-04 receivers with values using the latest IS-04 Receiver Caps syntax.

| ![Component Diagram of an IPMX Gateway Decoder](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/ipmx_decoder_component_diagram.iuml) |
|:--:|
...
Let’s choose an initialization Use-Case for the IPMX Gateway Encoder. When an IPMX Gateway Encoder starts up, it should present an EDID to it’s HDMI monitor. Reefrring to Figure 1, we can create a sequence diagram for this use-case from the EDID processing perspective, as shown in Figure 5, with associated steps described more fully in Table 1.

| ![Sequence Diagram for IPMX Gateway Encoder Setup using EDID](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/encoder_setup.iuml) |
|:--:|
...
| ---  | ---         | ---         |
| 1: Put Media Essentials Set | The IPMX Control System puts the media essentials required video and audio formats using an IPMX Device Control API endpoint for media essentials. | RAML for the endpoints is provided as a separate document including specifications for Work-in-Progess elements such as media_essentions_set. |
| 2: Present EDID | The IPMX Device Controller sends the EDID information to the HDMI Physical Source using the device’s DDC. | |
| 3: Update Sources, Flows and Senders | The IPMX Device Controller updates the IS-04 Sources, Flows and Senders with information from the Media Essentials Set. | |
...
| ![Sequence Diagram for IPMX Gateway Decoder Setup using EDID](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/decoder_setup.iuml) |
|:--:|
...
| 3: Update IPMX Device Control API Destination | The IPMX Device Controller updates the IPMX Device Control API Destination resource using EDID info | The destination resource associated with the IPMX Device Control API includes such information as Manufacturer, Serial Id, etc for the HDMI Monitor. Details provided in a WIP schema area. |
| 4: Update Receivers | The IPMX Device Controller updates the IS-04 Receivers with extended receiver caps. | |
...
| ![Sequence Diagram for User-Story 1 from NMOS Perspective](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/connection_management_sequence.iuml) |
|:--:|
...
Figure 8 shows the EDID processing required for this User Story. The IPMX Device Controller is responsible for detecting a change in the video format sent by the HDMI source by monitoring changes to HDMI infoframes sent by the source. The IPMX Device Controller can either poll the changes in HDMI infoframes or cue off of an HDMI Hot-plug-detect event from the HDMI Source.

| ![Sequence Diagram for User Configured Graphics Mode](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/infoframe_change_sequence.iuml) |
|:--:|
...
| 2: Detect InfoFrame Change | The IPMX Device Controller is responsible for detecting the change in inforframes | Implementation of this detection is vendor specific but typically handled via a HPD interrupt or polling of the HDMI driver for the device’s HDMI input port. |
...
| ![Sequence Diagram for Pro AV User Retrieving Raw and Parsed EDID Information](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/AMWA-TV/nmos-edid-connection-management/plantuml/docs/images/edid_retrieving_sequence.iuml) |
|:--:|
...
| 3: Update IPMX Device Control API Destination | The Destination is updated. | |
